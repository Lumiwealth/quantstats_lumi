...
def monthly_heatmap_detailedview(
    ...
    annot_drawdowns = pivot_drawdowns.map(
        lambda x: f"{x:.2f}" if _pd.notnull(x) else ""
    )
        ...
        for j in range(pivot_returns.shape[1]):
            if _pd.notnull(pivot_returns.iloc[i, j]):
                ...
                ax.text(
                    j + 0.5,
                    i + 0.55,
                    annot_drawdowns.iloc[i, j],
                    ha="center",
                    va="top",
                    fontsize=return_font_size * monthly_dd_font_rate,
    ...
    for idx, label in enumerate(ytick_labels):
        ax.text(
            ...
            fontsize=annot_size
            * 1.0,  # Maybe, 1.0 can be new argument like ytick_font_rate
        ...
        ax.text(
            ...
            horizontalalignment="right",
            fontsize=annot_size
            * annual_dd_font_rate,  # Set Drawdown font size slightly smaller
            transform=ax.transData,
            color="dimgray",
        )
...
def calculate_monthly_drawdowns(returns):
    drawdowns = []
    monthly_last_date = (
        returns.resample("ME").apply(lambda x: x.index[-1]).index.tolist()
    )
    monthly_last_trading_date = (
        returns.resample("ME").apply(lambda x: x.index[-1]).tolist()
    )
